...
Whenever a ``LvArray::ChaiBuffer`` is moved between memory spaces it will print some information to standard out including a name which can be set with the ``setName`` method. If this behavior is not desired it can be disabled with ``chai::ArrayManager::getInstance()->disableCallbacks()``.
