...
class PythonArrayTests(unittest.TestCase):
    ...
    def test_sorted_array_init(self):
        for lowerbound, upperbound in ((0,10), (1,5), (-15, 15), (100, 200)):
            array_from_c = lvarray.set_sorted_array(lowerbound, upperbound)
    ...
    def test_sorted_array_modification(self):
        for lowerbound, upperbound in ((0,10), (1,5), (-15, 15), (100, 200)):
            for factor in range(2, 6):
                ...
                self.assertTrue((array_from_c==array_from_python).all())
                self.assertTrue((lvarray.get_sorted_array()==array_from_python).all())
                self.assertTrue((lvarray.get_sorted_array()==array_from_c).all())

    def test_create_np_array(self):
        for lowerbound, upperbound in ((0,10), (1,5), (-15, 15), (100, 200)):
            array_from_c = lvarray.create_np_array(lowerbound, upperbound)
    ...
    def test_create_np_array_bad_input(self):
        self.assertEqual(lvarray.create_np_array(10, -1), None)
        with self.assertRaises(TypeError):
            lvarray.create_np_array("hello", 2)
            lvarray.create_np_array(1, 6.77)
